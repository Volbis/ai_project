...
Utilize les fichiers YOLOv5 locaux directement.
"""
...
app = FastAPI(title="ZKA Detection API", description="API de détection d'objets en temps réel avec IA", version="1.0.0")
    ...
    if model_name not in models_cache:
        try:
            ...
            if not model_path.exists():
                print(f"⚠️  {model_path} non trouvé, utilization de yolov5s.pt")
                model_path = YOLOV5_ROOT / "yolov5s.pt"

            if not model_path.exists():
                raise FileNotFoundError(
                    f"Modèle non trouvé: {model_path}. Téléchargez-le depuis https://github.com/ultralytics/yolov5/releases"
                )
...
def process_image(image: Image.Image, model_name: str = "yolov5s", conf_threshold: float = 0.25):
    ...
    try:
        ...
        # Letterbox
        img_letter, _ratio, (_dw, _dh) = letterbox(img_bgr, img_size, auto=False, scaleup=True)
        ...
        if pred[0] is not None and len(pred[0]) > 0:
            ...
            for *xyxy, conf, cls in pred[0]:
                ...
                detections.append(
                    {"bbox": [x1, y1, x2, y2], "confidence": confidence, "class": class_name, "class_id": class_id}
                )
        ...
        print(f"✅ Détection terminée: {len(detections)} objects trouvés en {processing_time:.3f}s")
...
@app.post("/detect")
async def detect_image(file: UploadFile = File(...), model: str = "yolov5s", confidence: float = 0.35):
...
async def get_statistics():
    ...
    avg_processing_time = (
        sum(statistics["processing_times"]) / len(statistics["processing_times"])
...
async def get_history(limit: int = 50):
    ...
    return JSONResponse({"history": detection_history[-limit:]})
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("⚠️  Le serveur démarre quand même, le modèle se chargera à la première utilization\n")

    print("🔗 Documentation API: http://localhost:8001/docs")
    print("🌐 Application ZKA: http://localhost:8001")
