...
async function startWebcamDetection() {
  try {
    ...
    console.log("🎥 Webcam démarrée, connection WebSocket...");
...
function connectWebSocket() {
  ...
  ws.onclose = () => {
    ...
    if (isWebcamActive) {
      // Réessayer la connection si la webcam est toujours active
      setTimeout(() => {
  ...
  if (data.detections.length === 0) {
    liveDetections.innerHTML = `
            <div class="text-center py-8">
                ...
                <p class="text-gray-500 dark:text-gray-400 font-medium">Aucun object détecté</p>
                <p class="text-xs text-gray-400 dark:text-gray-500 mt-2">
                    Montrez des personnes, animaux, ou objects à la caméra
                </p>
            </div>
        `;
  } else {
    ...
    liveDetections.innerHTML = Object.entries(grouped)
      .map(
        ([className, dets]) => `
...
async function processUploadedImages() {
  ...
  try {
    ...
    processBtn.innerHTML = '<i class="fas fa-magic mr-2"></i>Analyzer les images';
  }
}
        ...
        <div class="border border-gray-200 dark:border-gray-700 rounded-lg p-4 mb-4">
              ...
              result.detections && result.detections.length > 0
                ...
                <div class="bg-green-50 dark:bg-green-900 rounded-lg p-3 mb-2">
                    <h5 class="font-bold text-green-800 dark:text-green-200 mb-2 flex items-center">
                        ...
                        ${result.detections.length} object(s) détecté(s)
                    </h5>
                    <div class="space-y-1">
                        ${result.detections
                          .map(
            ...
            `
                ...
                <div class="bg-yellow-50 dark:bg-yellow-900 rounded-lg p-4 text-center">
                    ...
                    <p class="text-yellow-800 dark:text-yellow-200 font-medium">Aucun object détecté</p>
                    <p class="text-sm text-yellow-700 dark:text-yellow-300 mt-1">
                        YOLOv5 détecte : personnes, animaux, véhicules, objects du quotidien
                    </p>
                    <p class="text-xs text-yellow-600 dark:text-yellow-400 mt-2">
                        💡 Essayez avec une photo contenant des personnes, voitures, ou objects physiques
                    </p>
                </div>
            `
            }
